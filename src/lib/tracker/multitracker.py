...
class JDETracker(object):
    ...
    def update(self, im_blob, img0):
        ...
        for track in self.tracked_stracks:
            if not track.is_activated:  # 已创建但是还未激活
        ...
        # print(dists)
        '''for i,j in zip(detections,r_tracked_stracks):
            print("det:" ,i.tlwh)
            print("track:", j.tlwh)'''
        ...
        for it in u_unconfirmed:  # 还未被激活的track miss直接删除
        ...
        for inew in u_detection:
            ...
            if track.score < self.det_thresh:
                continue
            track.activate(self.kalman_filter, self.frame_id)  # 初始化
            activated_starcks.append(track)
        """ Step 5: Update state"""
...
def sub_stracks(tlista, tlistb):  # 在a中，不在b中的
    stracks = {}
    for t in tlista:
