...
def create_model(opt):
  num_layers = int(opt.arch[opt.arch.find('_') + 1:]) if '_' in opt.arch else 0
  arch = opt.arch[:opt.arch.find('_')] if '_' in opt.arch else opt.arch
  get_model = _model_factory[arch]
  model = get_model(opt= opt,num_layers=num_layers)
  return model
