...
from utils.image import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian,draw_ellipse
...
class JointDataset(LoadImagesAndLabels):  # for training
    ...
    def __getitem__(self, files_index):
        ...
        for k in range(num_objs):
            label = labels[k]
            bbox = label[2:]
            '''roi = copy.deepcopy(bbox)
            roi[[0, 2]] = roi[[0, 2]] * imgs.shape[2]
            roi[[1, 3]] = roi[[1, 3]] * imgs.shape[1]
            roi[0] = roi[0] - roi[2] / 2.
            roi[1] = roi[1] - roi[3] / 2.
            roi[2] = roi[0] + roi[2]
            roi[3] = roi[1] + roi[3]'''
            #print(files_index, label, img_path, roi)
            cls_id = int(label[0])
            ...
            #bbox_xy[2] = bbox_xy[0] + w
            #bbox_xy[3] = bbox_xy[1] + h
            ...
            if h > 0 and w > 0:
                ...
                #draw_gaussian(hm[cls_id],ct_int,int(h),int(w))

                if self.opt.ltrb:
                    wh[k] = ct[0] - bbox_amodal[0],ct[1] - bbox_amodal[1], bbox_amodal[2] - ct[0],bbox_amodal[3] - ct[1]
